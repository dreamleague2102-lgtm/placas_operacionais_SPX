...
function buildShopeeCard(codigo, numero, rodape, preenchida = true) {
  ...
  div.innerHTML = `
    ...
    <div class="ws-body">
      ...
      <div class="ws-qr"></div>
      <div class="ws-posto" style="font-size:${inferior.fonte}px">${preenchida ? escHtml(rodape) : ''}</div>
    </div>
...
function tamanhoFonteNomeCompacta(linhas, configuracao = null) {
  const base = tamanhoFonteNomeSelecionado(linhas, configuracao);
  const automatico = configuracao ? configuracao.fonteAuto !== false : (document.getElementById('nome-quatro-tamanho-auto')?.checked ?? true);
  if (!automatico) return base;
  const maiorLinha = Math.max(...linhas.map(linha => String(linha).length), 1);
  return Math.max(6, Math.min(base, Math.floor(600 / maiorLinha)));
}
...
function renderNomeDuploPreview(area) {
  ...
  folha.innerHTML = exibidos.map(item => {
    ...
    return `<div style="position:relative;border:1.5px solid #777;overflow:hidden;color:#000;">
      ...
      <div style="position:absolute;inset:29px 20px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:3px;font-weight:${item?.negrito === false ? 400 : 700};font-size:${fonte}px;line-height:.96;font-family:'${item?.familia || 'Calibri'}',Arial,sans-serif;overflow:hidden;">${linhas.map(linha => `<span style="display:block;width:100%;max-width:100%;min-width:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;">${escHtml(linha)}</span>`).join('')}</div>
    </div>`;
  }).join('');
...
function renderNomeQuatroPreview(area) {
  ...
  folha.innerHTML = itens.map(item => {
    ...
    return `<div style="position:relative;border:1.5px solid #777;overflow:hidden;color:#000;">
      ...
      <div style="position:absolute;inset:26px 16px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:3px;font-weight:${item?.negrito === false ? 400 : 700};font-size:${fonte}px;line-height:.94;font-family:'${item?.familia || 'Calibri'}',Arial,sans-serif;overflow:hidden;">${linhas.map(linha => `<span style="display:block;width:100%;max-width:100%;min-width:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;">${escHtml(linha)}</span>`).join('')}</div>
    </div>`;
  }).join('');
  area.appendChild(folha);
}
  ...
  const placa = (item) => {
    ...
    <div style="width:100%;height:100%;position:relative;overflow:hidden;">
      ...
      <div style="position:absolute;inset:.48in .34in;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden;
        ...
        ${linhas.map(linha => `<div style="display:block;width:100%;max-width:100%;min-width:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;">${escHtml(linha)}</div>`).join('')}
      </div>
    </div>
  </section>`;
  };
...
document.getElementById('print-nome-quatro').addEventListener('click', () => {
  ...
  for (let i = 0; i < itens.length; i += 4) {
    ...
    const placas = grupo.map(item => {
      const linhas = item ? quebrarTextoPlaca(item.nome) : [];
      const fonte = item ? tamanhoFonteNomeCompacta(linhas, item) : 60;
      return `<div style="position:relative;border:1.5px solid #777;overflow:hidden;background:#fff;">
        ...
        <div style="position:absolute;inset:.32in .22in;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:.03in;font-weight:${item.negrito === false ? 400 : 700};font-size:${fonte}pt;line-height:.94;font-family:'${item.familia || 'Calibri'}',Arial,sans-serif;color:#000;text-transform:uppercase;overflow:hidden;">${linhas.map(linha => `<span style="display:block;width:100%;max-width:100%;min-width:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;">${escHtml(linha)}</span>`).join('')}</div>` : ''}
      </div>`;
    }).join('');
...
document.getElementById('print-nome-duplo').addEventListener('click', () => {
  ...
  for (let i = 0; i < itens.length; i += 2) {
    ...
    const placas = dupla.map(item => {
      const linhas = item ? quebrarTextoPlaca(item.nome) : [];
      const fonte = item ? tamanhoFonteNomeSelecionado(linhas, item) : 70;
      return `<div style="height:3.65in;position:relative;border:1.5px solid #777;overflow:hidden;background:#fff;">
        ...
        <div style="position:absolute;inset:.36in .28in;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:.03in;font-weight:${item?.negrito === false ? 400 : 700};font-size:${fonte}pt;line-height:.96;font-family:'${item?.familia || 'Calibri'}',Arial,sans-serif;color:#000;text-transform:uppercase;overflow:hidden;">${linhas.map(linha => `<span style="display:block;width:100%;max-width:100%;min-width:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;">${escHtml(linha)}</span>`).join('')}</div>` : ''}
      </div>`;
    }).join('');
...
['nome-duplo-texto', 'nome-quatro-texto'].forEach(id => {
